  ...
  actions: {
    load() {
      try {
        ...
        if (raw) this.tokens = { ...defaultTheme, ...JSON.parse(raw) } as ThemeTokens;
      } catch {}
    },
    save() {
      try { localStorage.setItem(LS_KEY, JSON.stringify(this.tokens)); } catch {}
    },
    set<K1 extends keyof ThemeTokens>(key: K1, value: ThemeTokens[K1]) {
      // @ts-ignore
      this.tokens[key] = value;
      this.save();
    },
    setColor(name: keyof ThemeTokens['colors'], value: string){
      this.tokens.colors[name] = value;
      this.save();
    },
    setFont(name: keyof ThemeTokens['fonts'], value: string){
      this.tokens.fonts[name] = value;
      this.save();
    },
    setRadius(name: keyof ThemeTokens['radii'], value: string){
...
export function themeVariablesStyle(tokens: ThemeTokens){
  return `:root{\n`
