...
function buildPricingHtml(t) {
    const ff = fontStack(t.fontFamily);
    return `<section class=\"py-24\"><div class=\"px-6 max-w-5xl mx auto text-center mb-14\"><h2 class=\"text-4xl font-bold mb-4 text-gray-900 dark:text-white\" style=\"font-family:${ff}\">${t.title}</h2><p class=\"text-lg text-gray-600 dark:text-gray-300\" style=\"font-family:${ff}\">${t.subtitle}</p></div><div class=\"grid md:grid-cols-3 gap-8 max-w-5xl mx-auto\">${['Starter', 'Pro', 'Enterprise'].map((plan, i) => { const price = [t.priceA, t.priceB, t.priceC][i]; const highlight = i === 1; const badge = highlight ? `<div class=\\\"absolute -top-3 left-1/2 -translate-x-1/2 text-white text-xs px-3 py-1 rounded-full\\\" style=\\\"background:${t.primaryColor}\\\">${t.highlightLabel}</div>` : ''; return `<div class=\"twb-card p-6 ${t.rounded} text-center relative\" ${highlight ? `style=\\\"border-color:${t.primaryColor}\\\"` : ''}>${badge}<h3 class=\"text-xl font-semibold mb-4 text-gray-900 dark:text-white\" style=\"font-family:${ff}\">${plan}</h3><div class=\"text-3xl font-bold mb-4\" style=\"color:${highlight ? t.primaryColor : t.secondaryColor};font-family:${ff}\">${price}</div><a href=\"#\" class=\"twb-btn ${t.rounded} ${highlight ? 'twb-btn-solid' : 'twb-btn-outline'}\" style=\"${highlight ? `--btn-bg:${t.primaryColor}` : `--btn-color:${t.primaryColor}`};font-family:${ff}\">Choose Plan</a></div>`; }).join('')}</div></section>`;
}
...
function buildTestimonialsHtml(t) {
    ...
    const ff = fontStack(t.fontFamily);
    return `<section class=\"py-24 bg-gray-50 dark:bg-gray-900\"><div class=\"px-6 max-w-4xl mx-auto text-center mb-14\"><h2 class=\"text-4xl font-bold mb-4 text-gray-900 dark:text-white\" style=\"font-family:${ff}\">${t.title}</h2><p class=\"text-lg text-gray-600 dark:text-gray-300\" style=\"font-family:${ff}\">${t.subtitle}</p></div><div class=\"grid md:grid-cols-3 gap-8 max-w-5xl mx-auto\">${names.map((n, i) => { const color = [t.primaryColor, t.accentColor, t.secondaryColor || t.primaryColor][i % 3]; return `<div class=\"twb-card p-6 shadow-sm\"><div class=\"flex mb-4\">${Array.from({ length: 5 }).map(() => `<div class=\\\"w-4 h-4 mr-1 rounded\\\" style=\\\"background:${t.accentColor}\\\"></div>`).join('')}</div><p class=\"twb-text-muted mb-4 text-sm\">\"Amazing tool for rapid prototyping!\"</p><div class=\"flex items-center\"><div class=\"w-10 h-10 rounded-full mr-3\" style=\"background:${color}\"></div><div><div class=\"font-semibold text-gray-900 dark:text-white\" style=\"font-family:${ff}\">${n}</div><div class=\"text-xs twb-text-muted\">Customer</div></div></div></div>`; }).join('')}</div></section>`;
}
...
export const componentRegistry = [
    ...
    },
    {
        ...
        fields: [
            { key: 'labels', label: 'Tabs labels (comma)', type: 'textarea' },
            { key: 'activeIndex', label: 'Active index', type: 'text' },
            { key: 'variant', label: 'Style', type: 'select', options: [{ label: 'Underline', value: 'underline' }, { label: 'Cards', value: 'cards' }] },
            { key: 'rounded', label: 'Rounded', type: 'select', options: [{ label: 'Default', value: 'rounded' }, { label: 'lg', value: 'rounded-lg' }, { label: 'xl', value: 'rounded-xl' }] },
            { key: 'fontFamily', label: 'Font Family', type: 'select', options: [{ label: 'Inter', value: 'Inter' }, { label: 'Poppins', value: 'Poppins' }, { label: 'Roboto', value: 'Roboto' }, { label: 'Open Sans', value: 'Open Sans' }] },
            { key: 'primaryColor', label: 'Primary Color', type: 'color' }
        ],
        build: (t) => buildTabsHtml(t)
    },
    {
        id: 'modal',
...
function buildBlogHtml(t) {
    ...
    const ff = fontStack(t.fontFamily);
    return `<section class=\"py-24\"><div class=\"px-6 max-w-5xl mx-auto text-center mb-14\"><h2 class=\"text-4xl font-bold mb-2 text-gray-900 dark:text-white\" style=\"font-family:${ff}\">${t.title}</h2><p class=\"text-gray-600 dark:text-gray-300\" style=\"font-family:${ff}\">${t.subtitle}</p></div><div class=\"grid sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-5xl mx-auto\">${posts.map(p => `<article class=\"twb-card overflow-hidden\"><div class=\"h-28 bg-gray-200 dark:bg-gray-800\"></div><div class=\"p-4\"><h3 class=\"font-semibold mb-1 text-gray-900 dark:text-white\" style=\"font-family:${ff}\">${p}</h3><p class=\"text-xs twb-text-muted\">Lorem ipsum dolor sit amet.</p></div></article>`).join('')}</div></section>`;
}
...
function buildMediaTextHtml(t) {
    ...
    const ff = fontStack(t.fontFamily);
    return `<section class=\"py-24\"><div class=\"px-6 max-w-6xl mx-auto grid md:grid-cols-2 gap-10 items-center\"><div class=\"${orderA}\"><img src=\"${t.imageUrl}\" alt=\"\" class=\"w-full h-64 object-cover ${t.rounded}\"></div><div class=\"${orderB}\"><h2 class=\"text-3xl font-bold mb-3 text-gray-900 dark:text-white\" style=\"font-family:${ff}\">${t.title}</h2><p class=\"twb-text-muted mb-6\" style=\"font-family:${ff}\">${t.text}</p><a href=\"#\" class=\"twb-btn twb-btn-solid ${t.rounded}\" style=\"--btn-bg:${t.primaryColor};font-family:${ff}\">Learn more</a></div></div></section>`;
}
// Push into registry
componentRegistry.push({
    ...
    defaults: interactiveFaqDefaults,
    fields: [
        { key: 'title', label: 'Title', type: 'text' },
        { key: 'subtitle', label: 'Subtitle', type: 'textarea' },
        { key: 'questions', label: 'Questions (comma list)', type: 'text' },
    ...
    build: (t) => buildInteractiveFaqHtml(t)
});
