...
function toReactJsx(html: string){
  return html.replace(/class=\"/g,'className="')
}

export interface AggregatedFrameworks {
  vue: string
  react: string
  svelte: string
  angular: string
...
export interface BlockExport {
  html: string
  scripts?: Partial<Record<keyof AggregatedFrameworks, string>>
}

function ensureScriptTag(snippet: string){
  const isTag = /<script[\s>]/i.test(snippet.trim());
  return isTag ? snippet : `<script>\n${snippet}\n<\/script>`;
}

export function aggregatePageFrameworks(blocks: Array<string | BlockExport>): AggregatedFrameworks{
  const theme = useThemeStore()
  const vars = themeVariablesStyle(theme.tokens)
  const fonts = fontsLinks()
  const style = `<style>${vars}</style>`
  const htmls: string[] = []
  const scriptSets: Record<keyof AggregatedFrameworks, Set<string>> = {
    vue: new Set(), react: new Set(), svelte: new Set(), angular: new Set(), html: new Set()
  }

  for(const b of blocks){
    if(typeof b === 'string'){ htmls.push(b); continue; }
    htmls.push(b.html)
    if(b.scripts){
      for(const key of Object.keys(b.scripts) as (keyof AggregatedFrameworks)[]){
        const code = b.scripts[key]
        if(code) scriptSets[key].add(code)
      }
    }
  }
  ...
  const commentFor = (fw: keyof AggregatedFrameworks) => {
