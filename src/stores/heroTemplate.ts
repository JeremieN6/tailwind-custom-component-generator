export interface HeroTokens {
  ...
  // Background styles
  backgroundStyle: 'theme' | 'gradient' | 'solid' | 'image';
  gradientFrom?: string;
  gradientTo?: string;
  bgImageUrl?: string;
  overlayOpacity?: number; // only for backgroundStyle === 'image'
...
export function generateFrameworkWrappers(html: string, _tokens?: HeroTokens): MultiFrameworkCode {
